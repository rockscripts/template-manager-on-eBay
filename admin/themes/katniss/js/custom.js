...
jQuery(document).ready(function(){
	...
	function mainwrapperHeight() {
		...
		if(jQuery('.mainwrapper').height() < windowHeight)
            ...
            jQuery('.mainwrapper').css({minHeight: "100%"});
	}
	...
	jQuery('.skin-color').click(function(){
		...
		if(jQuery('#skinstyle').length > 0) {
			if(s!='default')
				jQuery('#skinstyle').attr('href','themes/katniss/css/style.'+s+'.css');	
			else 
				jQuery('#skinstyle').remove();
		} else {
			if(s!='default')
				jQuery('head').append('<link id="skinstyle" rel="stylesheet" href="themes/katniss/css/style.'+s+'.css" type="text/css" />');
		}
		return false;
	});
