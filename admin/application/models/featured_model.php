...
{ 
     function get_featured_listing($id_ebay_design,$user_id)
    {
        $data = array
            (
           ...
           $query = $this->db->get_where('feature_listings', $data);  
           $this->db->order_by('id_featured_listing', 'DESC');
         if($query->num_rows()>0)
             return $query->result();       
         else
             return false;
    }
     function add_featured_listing($id, $id_ebay_design,$title,$price,$link,$image_url,$user_id)
    {
        if($this->exist_feature_listing($id_ebay_design,$title,$price,$image_url) < 1):
            if($image_url=="ignore")
            {
                  $data = array
            (
            ...
            'image_url' => $image_url,
             ...
             if ( $id )
      {
        $this->db->where('id_featured_listing', $id );
        $this->db->update('feature_listings', $data );
      } else {
        $this->db->insert('feature_listings', $data);
      }
       return TRUE ;
       else:
           return false;
    ...
    function exist_feature_listing($id_ebay_design,$title,$price,$image_url)
      {
         $data = array
            (
            'id_ebay_design' => $id_ebay_design,
    ...
    function get_featured_listing_with_id($id_featured_listing)
    {
        $data = array
        (
        'id_featured_listing' => $id_featured_listing    
    ...
    public function delete_featured_listing($id_featured_listing)
    {
      $this->db->where('id_featured_listing', $id_featured_listing)->delete('feature_listings');
