<h4 class="widgettitle nomargin shadowed">Save New Arrival Listing</h4>
    ...
    <div class="widgetcontent bordered shadowed nopadding">
        ...
        <p class="stdformbutton">
            <div class="btn btn-primary" id="save_arrival" style="margin-left: 15px;"><i class="iconsweets-zip"></i>Save</div>
        </p>
...
<script>
    ...
    jQuery(document).ready(function() {
        ...
        var url = window.location.hostname === 'blueimp.github.io' ? 
                    '//jquery-file-upload.appspot.com/' : 'http://rockscripts.org/designmanager/server/php/?upload_path=store_designREPLACEarrivalsREPLACE'+id_user+'/',
        ...
        jQuery('#fileupload').fileupload({
          ...
          /*  data:{id:"3"} ,*/
            ...
            jQuery.each(data.result.files, function (index, file) {
               ...
               save_new_arrivals_listing(id,title,link,price,file.url.replace("http://"+window.location.hostname+"/designmanager/server/php","http://fileshosting."+window.location.hostname));   
               remove_error_field("name");
               remove_error_field("link");
               remove_error_field("price");   
               show_successfull_message("New Arrival Listing was saved successful.");
               jQuery("#save_arrival").fadeOut();
        ...
        jQuery("#save_arrival").click(function()
        {
                ...
                if (  validate_link( link ) )
                {
                    ...
                    if(jQuery('#files').find('div').size() >0) {
                        ...
                        jQuery("#files").find("button").first().trigger('click');
                          ...
                          if(id > 0)
                            {
                              save_new_arrivals_listing(id,title,link,price,'ignore');
                              show_successfull_message("New Arrival Listing was saved successful.");
                              remove_error_field("name");
                              remove_error_field("link");
                              remove_error_field("price"); 
