...
<div id="tab_container">
  ...
  <ul class="nav nav-tabs" id="myTab">
    <?php 
    if(!empty($home_tabs_array)):     
 
      foreach($home_tabs_array as $key => $tab):
    ?>
      <li class="<?php if ( $key == 0 ) echo 'active'; ?>">
        <a data-toggle="tab" href="#tab-<?=$tab->id_home_design_tab ?>" style="height:17px;">
            <div class="left">
                 <?=$tab->title ?>
            </div>

          <div action="edit" style="float:right;" id_tab="<?=$tab->id_home_design_tab ?>" title="Edit Image" class="button-controls custom_button" >
            <img src="<?= base_url() ?>themes/katniss/img/edit.png"  title="Edit Tab" >
          </div>
          <div action="delete" style="float:right;margin-left:20px;" id_tab="<?=$tab->id_home_design_tab ?>" title="Delete Image" class="button-controls custom_button">
  ...
  <div class="tab-content" id="myTabContent" style="margin-top: -20px;">
    ...
    if(!empty($home_tabs_array)):   
      foreach($home_tabs_array as $key => $tab):
    ?>
      <div id="tab-<?=$tab->id_home_design_tab ?>" class="tab-pane fade in <?php if ( $key == 0 ) echo 'active'; ?>" style="border:none;">    
               <p><?=$tab->content ?></p>
      </div>
    <?php 
      endforeach; 
...
jQuery(".button_new_tab").click(function()
 ...
 jQuery(".button_add_tab").click(function()
 {
    ...
    if(tab_title != "")
    {
     ...
     message    = home_design_add_tab(dataString);  
...
{   
    ...
    if(action=="delete")
    {
       ...
       if (result==true) 
        {
          jQuery(".loader-image").fadeIn();
          message =  home_design_delete_tab(id_tab); 
                ajax_load_home_design_tabs_form(function(response)
                {
                     form_tabs = callback_ajax_load_home_design_tabs_form(response);
                     jQuery("#content").html(form_tabs); 
    ...
    {      
      id_tab = jQuery(this).attr("id_tab"); 
      tab    = ajax_load_home_design_tab(id_tab); 
      jQuery('input[name=tab_id]').val(tab[0].id_home_design_tab);
